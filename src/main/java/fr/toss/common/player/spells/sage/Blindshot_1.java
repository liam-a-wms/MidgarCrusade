...
public class Blindshot_1 extends Spell {
	...
	{
		return I18n.format("spell.sage.blindshot");
	}
	...
	public boolean onUse()
	{
		...
		{
			if (p.getCurrentEquippedItem().getItem() instanceof ItemLightningStaff)
			{
				...
				player.getPlayer().addChatComponentMessage(new ChatComponentText(ChatColor.RED + "You must equip a staff for this." + ChatColor.RESET));
		}
		else
			player.getPlayer().addChatComponentMessage(new ChatComponentText(ChatColor.RED + "You must equip a staff for this." + ChatColor.RESET));
