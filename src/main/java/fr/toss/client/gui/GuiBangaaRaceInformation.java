...
public class GuiBangaaRaceInformation extends GuiScreen {
	...
	public GuiBangaaRaceInformation(String Race_name, String p_description[], String p_advices[], int px, int py, int p_color, int race_id)
	{
		super();
    ...
    protected void actionPerformed(GuiButton b)
    {
    	...
    	GuiSelectBangaaClass gui;
    	if (b.id == 42)
    	{
    		PacketSetRace packet;
    		
    		packet = new PacketSetRace(this.race_id);
    		Packets.network.sendToServer(packet);
    		try {
				Main.getPlayerClient().init(this.race_id, race_id);
			} catch (Exception e) {}
    		
    		gui = new GuiSelectBangaaClass();
